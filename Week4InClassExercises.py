...
#you can reference a global variable in a function but you cannot edit it without the global reference
x = 6
def foo():
	x = 20
	def bar():
...
def selection(args):
	...
	while i < 4:
		i += 1
		num = random.randint(0,len(args))
		compcolors.append(colors[num])
selection(colors)
print(compcolors)
usercolors = []
def guess():
	...
	while i < 4:
		i += 1
		userguess = input("Enter your color guess")
		usercolors.append(userguess)
guess()
print(usercolors)
def compare():
	correctcounter = 0
	...
	while i < 4:
		...
		if(usercolors[i] == compcolors[i]):
			...
			for j in range(i,len(compcolors)):
				if(usercolors[i] == compcolors[j]):
					counter += 1
	print()
